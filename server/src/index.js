...
io.on("connection", (socket) => {
  socket.on("join", (roomOwner) => {
    const { room, error } = addRoom({
      playerName: roomOwner,
      playerId: socket.id,
    });
    if (error) {
  ...
  socket.on("visitRoom", ({ roomId, player }) => {
    const { room, error } = updateRoomPlayers({
      roomId,
      player: { playerName: player, playerId: socket.id },
    });
    if (error) return;
    socket.join(room.roomData.roomOwner);
    io.to(room.roomData.roomOwner).emit("newStateRoom", room);

    const roomsToExit = Array.from(socket.rooms)
      .slice(1)
      .filter((r) => r !== room.roomData.roomOwner);

    roomsToExit.forEach((roomOwner) => {
      const roomToExit = exitRoom({ playerId: socket.id, roomOwner });
      if (roomToExit.room && !roomToExit.error) {
        socket.leave(roomOwner);
        io.to(roomToExit.room.roomData.roomOwner).emit(
          "newStateRoom",
          roomToExit.room
        );
      }
